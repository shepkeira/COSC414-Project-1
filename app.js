...
function bacteria_overloap(firstBacteria, secondBacteria) {
    ...
    var a_sqar = Math.pow(line_between_centers[0], 2);
    var b_sqar = Math.pow(line_between_centers[1], 2);
    var magnitude_of_line = Math.sqrt(a_sqar + b_sqar);
    //first ignore cases where the centers are further apart then their furthest points
    if(magnitude_of_line > (
            Math.max(firstBacteria['radius'][0]*firstBacteria['scale'][0], firstBacteria['radius'][1]*firstBacteria['scale'][1]) +
            Math.max(secondBacteria['radius'][0]*secondBacteria['scale'][0], secondBacteria['radius'][1]*secondBacteria['scale'][1])
    )) {
        return false;
    }
    var angle_from_point_fist_to_second = Math.abs(Math.atan(line_between_centers[0], line_between_centers[1]));
    var angle_from_point_second_to_first = (Math.PI/2) - angle_from_point_fist_to_second;
    // if bacteria overlapping more x-wise
    if(
        ...
        ) 
    {
        ...
        var combinded_radius = firstBacteria_radius_in_direction_of_secondBacteria + secondBacteria_radius_in_direction_of_second_Bacteria;

        if (combinded_radius < magnitude_of_line) {
            return false;
        }
        console.log('bacteria_collided');
        return true;
    // if bacteria overlapping more y-wise
    } else 
    {
...
var InitDemo = function() {
    ...
    canvas.onmousedown = function(ev) {
        ...
        for(var i = circles.length - 1; i >= 0; i--) {
            ...
            if (in_circle) {
                circles[i]['deleted'] = true; //TODO: when circle deleted add particle affect
                break;
            }
        }
    }
